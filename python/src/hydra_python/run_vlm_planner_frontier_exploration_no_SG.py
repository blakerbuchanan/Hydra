...
def main(cfg):
    ...
    for question_ind in tqdm(range(len(questions_data))):
        ...
        vlm_planner = hydra.VLMPLannerEQAGPTNoSG(
            cfg.vlm,
        ...
        for cnt_step in range(num_steps):
            if len(tsdf_planner.frontiers_to_sample) < 2:
                break
            
            click.secho(f"Overall step {cnt_step} and vlm step {planning_steps}",fg="green",)
            is_confident, confidence_level, answer_output = vlm_planner.get_next_action()
            ...
            for n_step in range(max_nav_steps):
