...
from voxel_mapping import GridParams
from voxel_mapping.voxel.voxel import SparseVoxelMap
from voxel_mapping.motion.space import XYT
from voxel_mapping.motion.robot import Footprint
from voxel_mapping.utils.geometry import angle_difference, interpolate_angles
from voxel_mapping.utils.morphology import (
    binary_dilation,
    binary_erosion,
    expand_mask,
...
class SparseVoxelMapNavigationSpace(XYT):
    ...
    def create_collision_masks(self, orientation_resolution: int, show_all: bool = False):
        ...
        self._footprint = Footprint(width=0.1, length=0.1, width_offset=0.0, length_offset=0.0)
    ...
    def get_frontier(
        ...
        obstacles, explored = self.voxel_map.get_2d_map()
        # rr.log(
        #     "world/voxel/obstacles_small",
        #     rr.Points3D(
        #         positions=np.array(occupancy_map_to_3d_points(obstacles, grid_origin, grid_resolution)),
        #         radii=0.05,
        #         colors=[255, 0, 0],
        #     ),
        # )
        # rr.log(
        #     "world/voxel/explored_og",
        #     rr.Points3D(
        #         positions=np.array(occupancy_map_to_3d_points(explored, grid_origin, grid_resolution)),
        #         radii=0.05,
        #         colors=[0, 0, 255],
        #     ),
        # )
        ...
        # Extract edges from our explored mask
        obstacles, less_explored = self._get_conservative_2d_map(obstacles, traversible)
        # rr.log(
        #     "world/voxel/conservative_obstacles",
        #     rr.Points3D(
        #         positions=np.array(occupancy_map_to_3d_points(obstacles, grid_origin, grid_resolution)),
        #         radii=0.05,
        #         colors=[150, 0, 0],
        ...
        # )
        # rr.log(
        #     "world/voxel/less_explored",
        #     rr.Points3D(
        #         positions=np.array(occupancy_map_to_3d_points(less_explored, grid_origin, grid_resolution)),
        #         radii=0.05,
        #         colors=[0, 0, 150],
        #     ),
        # )
        ...
        #     "world/voxel/edges",
        #     rr.Points3D(
        #         positions=np.array(occupancy_map_to_3d_points(edges, grid_origin, grid_resolution)),
        #         radii=0.05,
        #         colors=[0, 0, 150],
        #     ),
        # )
        ...
        # rr.log(
        #     "world/voxel/frontier_edges",
        #     rr.Points3D(
        #         positions=np.array(occupancy_map_to_3d_points(frontier_edges, grid_origin, grid_resolution)),
        #         radii=0.05,
        #         colors=[255, 255, 0],
        #     ),
        # )
        ...
        # rr.log(
        #     "world/voxel/expanded_frontier",
        #     rr.Points3D(
        #         positions=np.array(occupancy_map_to_3d_points(expanded_frontier, grid_origin, grid_resolution)),
        #         radii=0.05,
        #         colors=[0, 255, 0],
        #     ),
        # )
        ...
        #     rr.Points3D(
        #         positions=np.array(occupancy_map_to_3d_points(outside_frontier, grid_origin, grid_resolution)),
        #         radii=0.05,
        #         colors=[255, 255, 255],
        #     ),
        # )
        ...
        if debug:
            ...
            plt.savefig("/home/saumyas/catkin_ws_semnav/src/debug.png")

        return obstacles, explored, frontier, frontier_edges, traversible
    ...
    def sample_closest_frontier(
        ...
        for x, y, dist in sorted(zip(xs, ys, distances), key=lambda x: x[2]):
            ...
            # We really want to calculate theta from the previous point to the current point
            theta = math.atan2(
                outside_point[1] - point_grid_coords[0, 1],
                outside_point[0] - point_grid_coords[0, 0],
            )
    ...
    def push_locations_to_stack(self, locations):
    ...
    def update(self, z=0.):
        ...
        rr.log(
            "world/voxel/outside_frontier",
            rr.Points3D(
                positions=self.outside_frontier_points,
                radii=0.04,
                colors=[255, 255, 255],
            ),
        )
